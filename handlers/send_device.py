...
from database.orm_query import orm_send_device, orm_update_device
...
async def send_device(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(SendDevice.get_place)


@router.callback_query(SendDevice.get_place)
async def query_place(callback: CallbackQuery, state: FSMContext):
    if callback.data == 'change_place':
        ...
        await state.set_state(SendDevice.input_place)
...
@router.message(SendDevice.input_place)
...
async def get_device_comment(message: Message, state: FSMContext, session: AsyncSession):
    if message.text:
        ...
        data.update(add)
        result = await orm_send_device(session, data)
        if result:
